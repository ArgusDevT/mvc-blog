...
class LocalCachedUI implements localCachedInterfaces
{
    ...
    public static function clearChached($array)
    {
        foreach ($array as $key => $value) {
            if (file_exists($_ENV["CACHED_FOLDER"] . $value . ".cached")) {
                $path = $_ENV["CACHED_FOLDER"] . $value . ".cached";
                Utils::debugLog($path, "Path-logs");
                unlink($path);
            }
        }
